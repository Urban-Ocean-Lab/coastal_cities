...
#Clusters. Remove the dummy row of NA values.
...
##Calculate the total population living in coastal Urbanized Areas or Urban Clusters. This comes out to 637,474 people.
coastal_cities$Coastal_City_Population[coastal_cities$Definition %in% "Urban Area or Cluster"] <- 
  sum(coast_urban.df$pop_2018)
##Pretty positive this isn't working right

##Calculate the total population living in coastal Urbanized Areas. This comes out to 189,528.
coastal_cities$Coastal_City_Population[coastal_cities$Definition %in% "Urban Area"] <-
  sum(as.numeric(coast_urban.df$`2018_pop`[str_detect(coast_urban.df$Name, "Urbanized Area")]))

test <- coast_urban.df[str_detect(coast_urban.df$Name, "Urbanized Area"),]
